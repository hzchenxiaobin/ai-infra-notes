...
def build_nav(current_day: Optional[int] = None, weeks: Optional[list] = None,
    ...
    lines.append('<div class="nav-accordion-item">')
    lines.append('  <div class="nav-accordion-header">')
    lines.append('    <a class="nav-link week-link" href="../week2/index.html">Week 2：CUDA 进阶优化与性能分析</a>')
    lines.append('  </div>')
    lines.append('</div>')

    # Week 3 (expanded with days)
    lines.append('<div class="nav-accordion-item is-expanded">')
    lines.append('  <div class="nav-accordion-header">')
    week3_cls = "nav-link week-link active" if current_day is not None else "nav-link week-link"
    lines.append(f'    <a class="{week3_cls}" href="index.html">Week 3：Transformer 执行本质与算子手写</a>')
    lines.append('    <button class="nav-accordion-toggle" aria-label="收起/展开 Week 3" aria-expanded="true">▼</button>')
    lines.append('  </div>')
