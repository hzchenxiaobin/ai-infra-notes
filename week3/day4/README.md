...
6. 能对照昇腾解释为什么 CANN 内置 FlashAttention（避免手写 O(N²) 的标准版）
...
| Shared Memory（片上 SRAM） | L0 Buffer / UB | FlashAttention 的 tile 驻留 ≈ 昇腾 L0 Buffer 预加载 |
...
| QK^T / PV GEMM（compute-bound） | Cube Core 大矩阵乘 | 两者 GEMM 部分都走专用计算单元，compute-bound 一致 |
| ncu 的 `dram__bytes_read/write` | msprof 的 Memory Bandwidth | 两者都能测 HBM 实际读写量 |

**关键差异**：CUDA 教学版需要开发者手写标准 Attention 来理解 O(N²) 问题；昇腾 CANN 已内置 FlashAttention 算子，生产环境直接调用即可，无需手写标准版。但理解 O(N²) 的来源对跨平台调优都必要——"算法思想不变，硬件抽象重映射"。

> 💡 **一句话总结**：昇腾把 FlashAttention 做成内置算子（开发者直接调），CUDA 需要手写标准版来理解问题根源。两者在"HBM 是瓶颈、片上 SRAM 是解药"的认知上完全一致。
...
2. **Online Softmax 三公式**（Week 2 Day 12 已学）：
...
> 💡 提交后在 [LeetGPU Attention 题目](https://leetgpu.com/challenges/attention)上记录通过耗时，用 ncu 对比 naive 版（O(N²)）和 FlashAttention 版（O(Nd)）的 `dram__bytes_read` 差异。完整题解（含 online softmax 三公式推导、HBM 访问对比）见 [Attention 题解](../../LeetGPU/leetgpu-attention-solution.md)。
...
- [ ] 能对照昇腾解释为什么 CANN 内置 FlashAttention（避免手写 O(N²) 的标准版）
...
5. **为什么 FlashAttention 能把 IO 从 O(N²) 降到 O(Nd)？核心思想是什么？**
   ...
   - **前提条件**：需要足够大的 SRAM（A100 每 SM 100KB）容纳 Q tile + K/V tile
