...
3. 实现一个最小化 Transformer 推理引擎（Mini Engine），用 Day 16 的 Softmax/LayerNorm kernel 替换 PyTorch 官方算子
4. 验证自定义版与 PyTorch 版的端到端正确性（误差 < 1e-4），并对比 latency
5. 能解释为什么自定义算子通常比 PyTorch 慢（0.8x ~ 0.95x），以及什么场景下自定义才有优势
6. 能对照昇腾 CANN 解释自定义算子集成的方式（C++ Extension vs `aclOpCompile`）

> 💡 **为什么重要**：Day 16-18 我们手写了 Softmax/LayerNorm/Attention 三个 kernel，但它们都是"独立可执行文件"。真实工程中，kernel 必须接入推理框架（PyTorch / vLLM / TensorRT）才能端到端跑通。今天就是把"散装 kernel"组装成"能跑的引擎"——这是从"会写 kernel"到"能做系统"的工程能力跃迁。Day 20 会对这个引擎做端到端 profiling。

---

### 学前导读：为什么 kernel 要接入框架

Day 16 我们写的 `softmax_layernorm.cu` 是一个独立程序：`main()` 里手动 `cudaMalloc`、`cudaMemcpy`、调 kernel、`checkResult`。这在教学阶段没问题，但真实场景有三个问题：
...
**今天的任务**：把 Day 16 的 Softmax/LayerNorm kernel 封装成 PyTorch 可调用的 C++ Extension，接入一个最小 Transformer Block，用自定义算子替换 `F.softmax` / `F.layer_norm`，GEMM 仍用 `torch.mm`（cuBLAS）。然后对比"全 PyTorch" vs "自定义算子"的正确性和 latency。

> 💡 **一句话总结**：今天不优化 kernel 本身（那是 Day 17 做的），而是学"怎么把 kernel 塞进框架"——这是工程集成的标准流程。
...
![Mini Transformer Engine 架构](../website/images/mini_engine_architecture.svg)

Mini Engine 是一个最小化的 Transformer 单层推理引擎，设计目标：

1. 用 PyTorch 做张量管理（malloc/autograd 不需要）
2. 用自定义 CUDA kernel 替换 Softmax/LayerNorm（Day 16 实现）
...
![PyTorch C++ Extension 集成流水线](../website/images/cpp_extension_pipeline.svg)
...
at::Tensor softmax_forward(at::Tensor input) {
    ...
    launch_softmax(
        input.data_ptr<float>(),                            // ② 从 Tensor 提取裸指针
...
![Latency 对比 PyTorch 官方 vs 自定义算子](../website/images/latency_comparison.svg)
...
| 自定义（Day 16） | ~0.8x（更慢） | 逐元素加载 + 两次 reduce + 全程 FP32 |
| 自定义 + float4（Day 17） | ~0.95x（接近） | 向量化弥补部分差距 |

> ⚠️ **预期结果**：自定义算子可能比 PyTorch **慢**（0.8x），这正常——PyTorch 的 softmax/layernorm 已经过高度优化（Day 17 读过源码）。本周的目标是**理解算子集成流程**，不是超越官方实现。只有当官方实现**没有覆盖**你的场景时（如 FlashAttention 的分块 softmax），自定义才有性能优势。

##### 为什么自定义通常比 PyTorch 慢？

回顾 Day 17 的源码分析，PyTorch 官方实现有 4 个我们没做的优化：
...
| `at::Tensor` / `data_ptr` | `aclTensor` / 数据指针 | 张量抽象跨平台一致 |
| `getCurrentCUDAStream` | ACL stream | stream 概念跨平台一致 |

**关键差异**：CUDA 手写算子集成是学习性质（理解算子内部 + 集成流程）；昇腾 CANN 已内置优化算子，生产环境直接调用 `aclnnSoftmax` / `aclnnLayerNorm`。但"把 kernel 封装为框架可调用接口"的工程模式两者一致——这就是跨平台迁移的本质：**算法思想不变，硬件抽象重映射**。

> 💡 **一句话总结**：昇腾把 Softmax/LayerNorm 做成内置算子（开发者直接调），CUDA 需要手写 + C++ Extension 集成（学习目的）。两者在"封装 kernel 为框架接口"的工程模式上完全一致。
...
下面是带 launch wrapper + C++ Extension 绑定的完整 kernel 文件。它在 Day 16 的基础上增加了：① `launch_softmax`/`launch_layernorm` 封装层 ② `#ifdef WITH_TORCH` 的 PyTorch 绑定 ③ 独立 `main()` 验证。完整文件见 [kernels/softmax_layernorm_ext.cu](kernels/softmax_layernorm_ext.cu)。
...
```bash
# 独立编译（不依赖 PyTorch）
nvcc -o softmax_layernorm_ext kernels/softmax_layernorm_ext.cu -O3 -arch=sm_80
...
参考 Day 17 的 float4 优化，修改 `launch_layernorm` 内部的 kernel，把逐元素加载改为 `float4` 批量加载，对比 latency 变化。
...
| 错误 | 原因 | 解决方法 |
|------|------|---------|
| `load_inline` 编译报错找不到 `torch/extension.h` | PyTorch 未装或 CUDA 版本不匹配 | `pip install torch` 且确保 `torch.version.cuda` 与 nvcc 一致 |
| `data_ptr<float>()` 类型不匹配 | Tensor 不是 float32 | 用 `.float()` 转换：`x = x.float()` |
| 自定义 kernel 在错误 stream 执行 | 没传 `getCurrentCUDAStream()` | launch wrapper 必须接收并传递 stream |
| `Max diff > 1e-4` | kernel 有 bug 或形状不对 | 先用独立模式 `./softmax_layernorm_ext` 验证 kernel 正确性 |
| `reshape` 后 tensor 不连续 | `permute` 后未 `contiguous` | 加 `.contiguous()`：`attn_flat = attn.reshape(...).contiguous()` |
| `load_inline` 首次很慢 | JIT 编译耗时 | 正常（~30s），后续从 `~/.cache/torch_extensions` 加载（~0.1s） |
| nsys 看不到自定义 kernel | kernel 名字被 mangle | 用 `--kernel-name regex:softmax_kernel` 过滤 |

**调试技巧**：

- 先用独立模式（`nvcc` 编译）验证 kernel 正确性，再接入 PyTorch
- `print(attn.sum())` 检查 softmax 输出是否合理（每行和应为 1）
- `torch.cuda.synchronize()` 后再 `max_diff`，避免异步执行导致假错误
- `load_inline(verbose=True)` 打印编译命令，定位编译错误

---

### 验证 Checklist

- [ ] 能用 `load_inline` 把 Day 16 kernel 集成到 PyTorch（`my_ops.softmax_forward` / `layernorm_forward`）
...
Day 19 我们把 Day 16 的 Softmax/LayerNorm kernel 封装为 PyTorch C++ Extension，接入了 Mini Transformer Engine：
...
6. **昇腾对照**：C++ Extension ↔ `aclOpCompile`，"封装 kernel 为框架接口"的工程模式跨平台一致

掌握这套集成流程后，任何自定义 CUDA kernel 都能接入 PyTorch。Day 20 会对 Mini Engine 做端到端 profiling，定位瓶颈算子和 fusion 机会。
