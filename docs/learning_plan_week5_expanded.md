...
|---------|------------|---------|
| Prefill 阶段 | Prefill 阶段 | 模型层面概念，与硬件无关 |
| Decode 阶段 | Decode 阶段 | 同上 |
| TTFT / TBT | 首token时延 / 逐token时延 | 指标定义一致 |
| `torch.profiler` | msprof | 都提供算子级时间线 |
| KV Cache | KV Cache | 概念完全一致 |
| 小 GEMM (M=1) | Vector Core 利用率低 | 与 CUDA SM 空闲同理 |
...
|---------|------------|---------|
| KV Cache | KV Cache | 概念完全一致 |
| 静态分配 | 静态 Cache 分配 | 两者都支持 |
| 动态分配 | 动态 Cache 扩展 | 两者都支持 |
| PagedAttention | 昇腾 PagedAttention | CANN 已内置类似实现 |
| KV Cache 量化 | KV Cache INT8/FP8 量化 | 跨平台一致 |
| 多轮对话复用 | 历史 Cache 复用 | 策略一致 |
...
> **时间分配**：早间1.5h（源码阅读1h + 昇腾对照30min）+ 晚间1h（架构图绘制）
...
## 昇腾对照
- 昇腾 CANN 内置 PagedAttention
- 概念完全一致
```

#### 练习题

**练习1（基础）**：手动构造一个 block table 示例：序列长度 50，block size=16，给出逻辑/物理映射。

**练习2（进阶）**：解释 copy-on-write 在 beam search 中的作用。

**练习3（综合）**：阅读 `vllm/core/block_manager.py` 中的 `BlockSpaceManager`，画出其类图。

---

### 今日面试题

**面试题1**：什么是 PagedAttention？它解决了什么问题？（⭐⭐⭐⭐⭐ 必考）

**参考答案要点**：
- PagedAttention 借鉴 OS 虚拟内存分页思想，将 KV cache 分成固定大小的 block
- 逻辑 block 号对序列连续，物理 block 号在显存中可以不连续
- 通过 block table 维护逻辑到物理的映射
- 解决的问题：
  1. 静态分配的内存浪费
  2. 动态分配的外部碎片
  3. 长文本场景下的显存管理困难
- 带来的好处：提高显存利用率，支持更大 batch、更长序列

**面试题2**：PagedAttention 中的 Copy-on-Write 是什么？在什么场景下使用？（⭐⭐⭐⭐ 高频）

**参考答案要点**：
- Copy-on-Write（写时复制）：多个 sequence 共享同一个物理 block 时，读共享、写复制
- 使用场景：
  - Beam search：多个候选序列共享 prompt 部分
  - 并行采样：同一个 prompt 生成多个不同输出
  - 多轮对话：共享历史对话上下文
- 优点：避免不必要的 KV cache 复制，节省显存
- 实现：在 block table 中标记引用计数，写入前检查是否共享，若共享则分配新 block 并复制

---

### 今日自测清单

- [ ] 能理解 PagedAttention 的核心思想（分页、逻辑/物理 block）
- [ ] 能画出 block table 的示意图
- [ ] 理解 copy-on-write 的原理和应用场景
- [ ] 能找到 vLLM 中的 BlockSpaceManager 源码
- [ ] 理解 Worker 如何构建 attention metadata 并传入 kernel
- [ ] 能对照昇腾解释 PagedAttention 的对应实现
...
|---------|------------|---------|
| PyTorch model backend | 昇腾 NPU backend | 两者都是框架后端 |
| KV Cache class | CANN KV Cache | 概念一致 |
| Prefill/Decode 循环 | 昇腾推理引擎执行流程 | 一致 |
| Tokenizer | Tokenizer | 与硬件无关 |
| 单请求 → 多请求 | 单请求 → 多请求 | 后续扩展方向一致 |
...
| ncu/nsys 指标 | 昇腾 msprof 对应 | 含义 |
|---------|------------|------|
| TTFT | 首 token 时延 | Prefill 阶段总时间 |
| TBT | 逐 token 时延 | Decode 阶段单步时间 |
| Kernel gap | Kernel 间隙 | launch overhead |
| SM Utilization | AI Core 利用率 | 计算单元利用率 |
| Memory Throughput | Memory Bandwidth | 显存带宽利用率 |
...
## 附录B：昇腾→CUDA 第5周概念映射总表

| 维度 | CUDA/vLLM 概念 | 昇腾 CANN 概念 | 差异说明 | 迁移难度 |
|------|-------------|------------|---------|---------|
| **Prefill/Decode** | 两阶段划分 | 两阶段划分 | 与硬件无关 | ★ |
| **TTFT/TBT** | 首token/逐token时延 | 同样指标 | 定义一致 | ★ |
| **KV Cache** | KV Cache | KV Cache | 概念完全一致 | ★ |
| **静态/动态分配** | 静态/动态 | 静态/动态 | 一致 | ★ |
| **PagedAttention** | vLLM PagedAttention | 昇腾 PagedAttention | CANN 已内置 | ★★ |
| **Block Table** | 逻辑→物理映射 | 同样映射 | 一致 | ★ |
| **Copy-on-Write** | 写时复制 | 写时复制 | 一致 | ★★ |
| **LLMEngine** | vLLM Engine | 昇腾推理引擎 | 概念类似 | ★★ |
| **Scheduler** | vLLM Scheduler | 昇腾调度器 | 功能类似 | ★★ |
| **Worker** | vLLM Worker | 昇腾 Worker | 功能类似 | ★ |
| **Continuous Batching** | 动态批处理 | 动态批处理 | 一致 | ★ |
| **Preemption/Swapping** | 抢占/换出 | 抢占/换出 | 策略类似 | ★★ |
| **CUDA Graph** | CUDA Graph | 昇腾 Graph 优化 | 概念类似 | ★★ |
| **torch.profiler/nsys** | PyTorch/nsys | msprof | 工具对应 | ★★ |
