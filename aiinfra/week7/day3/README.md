...
6. 了解 **昇腾 CANN 对应实现**——投机采样、分块 prefill、前缀缓存均已支持
...
| 高级特性 | 昇腾 CANN 对应 | 支持状态 | 说明 |
|---------|------------|---------|------|
| Speculative Decoding | 昇腾投机采样 | ✓ 已支持 | CANN 内置 |
| Chunked Prefill | 昇腾分块 prefill | ✓ 已支持 | MindIE |
| Prefix Caching | 昇腾前缀缓存 | ✓ 已支持 | block 级别 |
| KV Cache 量化 | 昇腾 KV 量化 | ✓ 已支持 | INT8/FP8 |
| CUDA Graph | 昇腾 Graph | ✓ 已支持 | 静态图优化 |

> 💡 三大高级特性在昇腾 CANN 中均已支持，概念与 CUDA 侧完全对应。差异在底层 KV Cache 管理（PagedAttention vs 昇腾 block allocator）和算子实现。
...
6. **能对照昇腾解释这些高级特性的跨平台支持吗？**

   - 三大特性在昇腾 CANN 中均已支持：投机采样、分块 prefill、前缀缓存
   - 昇腾 MindIE 内置了对应实现，概念与 vLLM/SGLang 完全对应
   - 差异在底层：KV Cache 管理用 PagedAttention vs 昇腾 block allocator
   - 投机采样的分布对齐逻辑跨平台一致（接受/拒绝采样）
   - 昇腾还支持 KV Cache 量化（INT8/FP8）和 Graph 优化，与 CUDA Graph 对应
