...
5. 能对比 **vLLM / TensorRT-LLM / LightLLM / 昇腾 MindIE** 四大框架的调度策略、KV Cache 管理、性能/灵活性权衡，给出场景选型建议<br>
...
| 昇腾 NPU | **MindIE** | 昇腾原生、AscendC 算子优化 |
...
| PagedAttention | 昇腾 PagedAttention | block 化管理 |

> 💡 调度思想跨平台统一——Continuous/Inflight/Split Fuse 都是 iteration-level 调度。昇腾 MindIE 同样支持动态批处理和分块 prefill，差异只在底层算子（AscendC vs CUDA）和 KV Cache 存储介质（HBM vs 显存）。
...
创建文件 [kernels/chunked_prefill_simulator.py](kernels/chunked_prefill_simulator.py)，对比 naive 与 chunked 两种 prefill 策略的 decode 延迟曲线：
...
class Sequence:
    ...
    prefilled_tokens: int = 0       # 已 prefill 的 token 数（=prompt_len 时完成）
...
class PrefillScheduler:
    ...
    def __init__(self, max_token_budget=32, max_num_seqs=8,
                 chunk_size=8, use_chunked=False):
    ...
    def schedule_iteration(self):
        ...
        # 3. 从 waiting 加入新请求（chunked: 每轮一块; naive: 一次性）
        # 4. 执行 decode，记录延迟
        ...
```
...
> 💡 提交后在 [LeetGPU Segmented Prefix Sum](https://leetgpu.com/challenges/segmented-prefix-sum) 上记录通过耗时。完整题解（含分段 scan kernel、段边界 carry、与 Chunked Prefill 分块累积的类比）见 [Segmented Prefix Sum 题解](../../leetgpu/week6/day4/leetgpu-segmented-prefix-sum-solution.md)。
...
6. **四框架选型**：快速迭代选 vLLM、极致性能选 TensorRT-LLM、高并发选 LightLLM、昇腾选 MindIE
...
6. **能对照昇腾解释推理框架调度的跨平台一致性吗？**

   - 调度思想与硬件无关：Continuous/Inflight/Split Fuse 都是 iteration-level 调度，昇腾 MindIE 同样支持动态批处理和分块 prefill
   - 昇腾已适配 vLLM（昇腾版 vLLM），MindIE-Service 对应 LightLLM 的推理服务能力
   - 差异在底层：调度器实现语言、KV Cache 存储介质（HBM vs 显存）、attention 算子（AscendC vs cuBLAS）
   - Chunked Prefill / PagedAttention 等概念在昇腾上完全对应，可无缝迁移调度逻辑
