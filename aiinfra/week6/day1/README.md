...
6. 能对照昇腾 CANN 解释 Dynamic Batching 的跨平台一致性，理解 timeout / max_batch_size 的 SLA 调参
...
![No Batching vs Static vs Dynamic Batching](../website/images/batching_strategies_comparison.svg)
...
![Dynamic Batcher 工作流程](../website/images/dynamic_batcher_flow.svg)
...
![Throughput vs Latency 曲线与饱和点](../website/images/throughput_latency_curve.svg)
...
| Throughput-Latency 曲线 | 吞吐-延迟曲线 | 概念一致 |

> 💡 Dynamic Batching 是**系统调度层面**的概念，与硬件无关——无论 CUDA 还是昇腾，都是"请求入队 → 凑批 → forward"。差异只在底层 forward 的 kernel 实现（CUDA GEMM vs 昇腾 AscendC 算子）。
...
6. **能对照昇腾解释 Dynamic Batching 的跨平台一致性吗？**

   - Dynamic Batching 是系统调度层面的概念，与硬件无关——无论 CUDA 还是昇腾，都是"请求入队 → 凑批 → forward → 返回"
   - 差异只在底层 forward 的 kernel 实现：CUDA 调 cuBLAS/cuDNN，昇腾调 AscendC 算子
   - timeout / max_batch_size / padding 策略 / 长度分组等调参方法跨平台通用
   - 昇腾推理框架（MindIE 等）同样支持动态批处理，概念完全一致
