...
| **GPU Utilization** | SM / AI Core 利用率 | 硬件使用率（饱和点时 ≈100%） |
...
![Benchmark 六大指标与两种测试方法](../website/images/benchmark_metrics_overview.svg)
...
> 💡 昇腾对应 `msprof` 工具，看 AI Core 利用率 / kernel 间隙，思路与 nsys 一致。

#### 6.5 昇腾对照

| 指标 | 昇腾对应 | 含义 |
|---------|------------|------|
| Throughput | 吞吐（tok/s） | 一致 |
| Latency | 时延 | 一致 |
| SM Utilization | AI Core 利用率 | 计算单元利用率 |
| Kernel gap | Kernel 间隙 | 调度开销 |
| nsys | msprof | profiling 工具 |
| ncu | msprof --kernel | 单算子分析 |

> 💡 性能指标跨平台统一——throughput/latency/利用率的概念无论 CUDA 还是昇腾都一致，差异只在 profiling 工具（nsys/ncu vs msprof）和底层算子。
...
6. **能对照昇腾解释 benchmark 指标的跨平台一致性吗？**

   - 指标跨平台统一：throughput/latency/TTFT/TPOT 无论 CUDA 还是昇腾都一致
   - 昇腾用 `msprof` 替代 nsys/ncu，看 AI Core 利用率（对应 SM util）、kernel 间隙
   - 饱和点判定方法相同：throughput 封顶 + latency 飙升 + util ≈100%
   - 差异在瓶颈优化：CUDA 侧用 cuBLAS/TensorRT，昇腾侧用 AscendC 算子/MindIE
