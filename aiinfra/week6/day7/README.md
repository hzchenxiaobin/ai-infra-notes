...
> 💡 **LLM 推理服务标配组合**：Continuous Batching + PagedAttention + Chunked Prefill。vLLM / TensorRT-LLM / LightLLM / 昇腾 MindIE 均支持此组合，差异在实现语言与 KV Cache 管理。
...
**Continuous Batching（Day2）**
3. Continuous Batching 和 Dynamic Batching 的区别？
4. Continuous Batching 为什么适合 LLM 推理？
5. Prefill + Decode 混合调度的挑战？

**vLLM Scheduler（Day3）**
6. vLLM Scheduler 的 schedule() 流程？
7. SchedulingBudget 的两个核心参数？
8. Preemption 的两种模式？默认哪个？为什么？

**框架对比（Day4）**
9. Inflight Batching 和 Continuous Batching 区别？
10. Chunked Prefill 是什么？解决什么问题？
...
**Benchmark（Day6）**
13. 如何做 throughput-latency benchmark？
14. 如何识别饱和点？
...
5. 跨平台：昇腾对照（MindIE/动态批处理/分块 prefill 概念一致）
```
...
4. **调度策略如何选择？给出你的决策流程**
   ...
   - 选型还考虑：实现语言（Python 灵活 vs C++ 快）、生态（vLLM 大 vs TRT-LLM 官方）、硬件（CUDA vs 昇腾）
...
6. **能对照昇腾解释 Week 6 调度策略的跨平台一致性吗？**

   - 调度思想与硬件无关：Continuous/Inflight/Split Fuse 都是 iteration-level，昇腾 MindIE 同样支持
   - 7 种策略在昇腾上完全对应：动态批处理=Dynamic、Inflight=Continuous、分块 prefill=Chunked、抢占=Preemption
   - 昇腾已适配 vLLM，MindIE-Service 对应推理服务能力
   - 差异在底层：调度器语言、KV Cache 存储（HBM vs 显存）、算子（AscendC vs cuBLAS）
   - benchmark 指标（throughput/latency/P99）跨平台统一，msprof 对应 nsys/ncu
...
- **昇腾 MindIE 文档**：动态批处理 / 分块 prefill / PagedAttention
