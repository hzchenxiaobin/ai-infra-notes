#!/usr/bin/env python3
"""
Build the Week 6 website from README.md (overview) and dayN/README.md (per-day).
Generates:
  - index.html: overview page (from week6/README.md)
  - dayN.html: one page per day (from week6/dayN/README.md)
Uses relative paths (../css/..., ../js/...) for shared resources,
since week6/ is one level below the deployment root on GitHub Pages.
"""
...
WEEK6_DIR = Path(__file__).parent.parent
...
def load_overview_and_days():
    """Load overview from week6/README.md and per-day markdown from week6/dayN/README.md."""

    readme_path = WEEK6_DIR / "README.md"
    if not readme_path.exists():
        raise FileNotFoundError(f"Week 6 README not found: {readme_path}")
    overview = readme_path.read_text(encoding="utf-8").replace("](website/images/", "](images/")

    day_title_pattern = re.compile(r"^## Day (\d+)[：:]\s*(.+)$")
    days = []
    for day_dir in sorted(WEEK6_DIR.glob("day*")):
    ...
    if not days:
        raise ValueError(f"No day*/README.md files found in {WEEK6_DIR}")
...
def build_nav(current_day: Optional[int] = None, weeks: Optional[list] = None,
    ...
    lines.append(f'<a class="{overview_class}" href="index.html">📌 Week 6 概览</a>')
    ...
    week_titles = {6: "Batching & 调度"}
    for week in weeks:
        week_titles[week["num"]] = week["title"]

    repo_root = WEEK6_DIR.parent
    existing_days = get_day_numbers(WEEK6_DIR)
    week_data = [
        {
            "num": 1,
            "href": "../index.html",
...
def page_template(title: str, nav_html: str, markdown: str,
    ...
    page_title = page_title if page_title is not None else f"Week 6 - {title}"
...
{nav_html}
    ...
    <script>
        ...
        try {{
            ...
            document.getElementById('content').innerHTML = '<div style="padding: 20px; color: #ff7b72; background: #2d1616; border-radius: 8px;">' +
                '<h2>⚠️ 页面渲染失败</h2>' +
                '<p>' + err.message + '</p>' +
