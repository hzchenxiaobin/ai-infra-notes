...
![FlashAttention Tiling：在 SRAM 中完成 Attention 计算](../../week4/website/images/flash_attention_tiling.svg)
...
![PagedAttention Block Table：逻辑连续、物理离散](../../week5/website/images/paged_attention_block_table.svg)
...
![vLLM 分层架构](../../week5/website/images/vllm_layered_architecture.svg)
...
![Continuous Batching Timeline：iteration 级动态进出](../../week8/website/images/continuous_batching_timeline.svg)
...
**题目概述**：给定课程总数 `numCourses` 和先修关系 `prerequisites`，判断能否完成所有课程（即图中是否有环）。

**与今日知识的关联**：课程表的 **拓扑排序** 与 vLLM Scheduler 的 **请求依赖调度** 同构——Scheduler 需要保证请求状态按正确顺序推进（WAITING → RUNNING → FINISHED），并处理抢占/恢复时的依赖一致性。两者都需要在 DAG 上维护合法执行顺序。面试中“如何检测循环依赖”和“如何调度有依赖关系的任务”是同一类问题。
...
> 💡 完整题解（含 Kahn 算法与 DFS 三色标记法对比、复杂度分析、与调度依赖的类比）见 [课程表题解](../../../leetcode/daily/week6/day4/课程表.md)。
