...
下面是完整可编译的 kernel 实现。代码分三部分：① 复用 Week 2 的 warp 原语 ② 搭出 blockReduceSum / blockReduceMax ③ Softmax kernel（三遍扫描）+ LayerNorm kernel（两次 reduce）。完整文件见 [kernels/softmax_layernorm.cu](kernels/softmax_layernorm.cu)。
...
Host 端的验证逻辑（`cpuSoftmax` / `cpuLayerNorm` / `checkResult` / `main`）见 [kernels/softmax_layernorm.cu](kernels/softmax_layernorm.cu) 文件后半部分，核心是：随机初始化 `M=128, D=1024` 的矩阵，分别跑 GPU kernel 和 CPU 参考，用 `maxDiff < 1e-5` 判定 PASS。
...
for (int i = tid; i < D; i += blockDim.x) local_max = fmaxf(local_max, in_row[i]);
// Pass 2: 再读 in_row[i] 求 sum
for (int i = tid; i < D; i += blockDim.x) local_sum += expf(in_row[i] - row_max);
// Pass 3: 第三次读 in_row[i] 写出
for (int i = tid; i < D; i += blockDim.x) out_row[i] = expf(in_row[i] - row_max) * inv_sum;
```
...
> 💡 提交后在 [LeetGPU Softmax 题目](https://leetgpu.com/challenges/softmax)上记录通过耗时，用 ncu 对比不同 `D` / `threads` 的性能差异。完整题解（含 online 两遍扫描优化、Roofline 分析）见 [Softmax 题解](../../leetgpu/leetgpu-softmax-solution.md)。
...
| 错误 | 原因 | 解决方法 |
|------|------|---------|
| Softmax 结果全 NaN | 没减 max，`exp(大数)` 溢出 | 检查 Step 1 是否求了 `row_max`，Step 2/3 是否用 `exp(x - row_max)` |
| Softmax 每行之和 ≠ 1 | `row_sum` 没广播或广播前没 `__syncthreads` | 确认 `if (tid==0) row_sum=...; __syncthreads();` 在归一化前 |
| LayerNorm 误差大（~1e-2） | reduce 用了 FP16 累加 | 全程用 FP32 做 reduce（`float local_sum`），混合精度时输入 cast 到 FP32 |
| `blockReduceSum` 结果只有部分对 | `numWarps` 算错或 warp0 读 smem 越界 | `numWarps = (blockDim.x + 31) / 32`；warp0 只读前 `numWarps` 个 |
| LayerNorm 方差算成负数 | 先除 N 再平方导致精度损失 | 必须先 `sum((x-mean)^2)` 再除 N，不要 `sum(x)/N` 后平方 |
| ncu 显示 DRAM 利用率很低 | L2 cache 命中率高（小矩阵）或访问不合并 | 增大 D 到 4096；确保 grid-stride 步长 = blockDim.x（coalesced） |
| 编译警告 `__shfl_down` deprecated | 用了旧版无 `_sync` API | 改用 `__shfl_down_sync(0xFFFFFFFF, val, offset)` |

**调试技巧**：

- 先用小矩阵（`M=1, D=32`，一个 warp）验证 warp 级 reduce 正确，再放大到 block 级
- 在 `blockReduceSum` 后让 lane 0 `printf` 出 `row_max` / `row_sum`，与 CPU 对比
- Softmax 验证额外检查：`Σ y[i] ≈ 1.0`（每行和应为 1）
- LayerNorm 验证额外检查：归一化后 `(y - β)/γ` 的均值≈0、方差≈1
