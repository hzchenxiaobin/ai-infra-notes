## Day 17：源码分析 —— PyTorch / FasterTransformer
...
4. 能列出工业级实现比 Day 16 手写版多的 3 个优化（向量化 / Welford / register 缓存），并评估各自收益
5. 理解 FP16 输入时 reduce 为什么要 cast 到 FP32（混合精度标准做法）
6. 完成至少一项优化（warp 级 Softmax 或 float4 LayerNorm），用 ncu 验证 DRAM Throughput 提升

> 💡 **为什么重要**：Day 16 我们手写了"能跑对"的 Softmax/LayerNorm，但和 PyTorch 官方实现比慢了 1.5-3x。今天通过读源码，找到差距在哪、为什么这么优化——这是从"能写 kernel"到"能写高性能 kernel"的关键一步。读源码不是为了背代码，而是建立"看到 memory-bound kernel 就条件反射想到向量化 + fusion + reduce 合并"的工程直觉。

---

### 学前导读：Day 16 的 kernel 跑对了，但慢在哪？

昨天我们用三遍扫描 + 两级 block reduce 实现了 Softmax/LayerNorm，与 CPU 误差 < 1e-5。但如果拿 `torch.softmax` 对比 latency，大概率会慢 1.5-2x。差距来自三个工程细节：

| 维度 | Day 16 手写版 | 差距来源 | 今日对应源码 |
|------|-------------|---------|------------|
...
今天的任务是**读官方源码 + 动手把 Day 16 的版本优化一两项**，用 ncu 量化收益。这不是为了超越 PyTorch（那需要 CUTLASS 级工程量），而是建立"看到 memory-bound 就知道优化在哪"的直觉。
...
Day 16 我们一律用 block 级（256 线程 + shared memory + `__syncthreads`）。但 PyTorch 在 D=1024 时改用 warp 级（32 线程，无 smem），原因有三：
...
> ⚠️ **注意**：warp 级的代价是"一个 warp 只能处理 D ≤ 1024 的行"（32 lane × 32 元素 = 1024）。D 更大时一个 warp 放不下，必须回退到 block 级。这就是 Day 16 用 block 级做"通用版"的原因——它什么 D 都能处理，但在常见 D（768/1024）上不是最优。

#### 3.2 向量化加载：float4 与 half2

![向量化加载：逐元素 vs float4 vs half2](../website/images/vectorized_load_comparison.svg)

这是 memory-bound kernel **最直接、收益最大**的优化。Day 16 我们逐元素加载：

```cuda
// Day 16：每次加载 1 个 float（32-bit）
...
Day 16 的 LayerNorm 用两次 reduce（先 mean 后 variance），读两遍 HBM。FasterTransformer 用 **Welford 在线算法**合并成一次遍历：
...
|------|-----------|------------|------|----------|
| Day 16（两次 reduce） | 3N（三遍扫描） | 2 | 1x（基准） | 低，清晰易读 |
...
Day 16 的版本在归一化阶段每次都从 HBM 读 gamma/beta。register 缓存避免了重复读，但要求 D 小到能放进 register（每线程 ~255 个 register，D=768 时每线程 ~3 元素，可行）。

---

### 昇腾对照

| PyTorch / FasterTransformer 优化 | 昇腾 CANN 对应 | 对照说明 |
|---------|------------|---------|
| warp 级 softmax（D≤1024） | Ascend C Softmax 算子向量化 | 昇腾 Vector Unit 天然处理向量，无需区分 warp/block 路径 |
| `float4` 向量化加载 | Vector Unit 一次处理多元素 | 昇腾 Vector 指令天然向量化，无需手动 `float4` cast |
| `__half2` 向量化（FP16） | Vector Unit half 向量 | 昇腾 Vector 指令支持 FP16 向量，天然高效 |
| FP32 reduce 保精度 | FP32 reduce 保精度 | 混合精度策略跨平台一致（FP16 输入→FP32 reduce→FP16 输出） |
...
**关键差异**：CUDA 的向量化需要开发者手动 `reinterpret_cast<float4*>` 并保证对齐；昇腾 Vector Unit 的指令天然是向量化的，开发者只需指定 repeat 数。但底层逻辑一致——都是"一次处理多元素"提升吞吐。

> 💡 **一句话总结**：跨平台迁移的本质是「算法思想不变，硬件抽象重映射」。Welford、向量化、FP32 reduce 这三件事在 CUDA 和昇腾上都要做，只是 CUDA 要手写、昇腾由算子库/指令封装。
...
下面是优化版 kernel，包含两项改进：① warp 级 Softmax（参考 PyTorch `softmax_warp_forward`）② float4 向量化 LayerNorm（参考 FasterTransformer 向量化加载）。同时保留 Day 16 的 block 级 / 标量版本做对比基准。完整文件见 [kernels/softmax_layernorm_opt.cu](kernels/softmax_layernorm_opt.cu)。
...
对比 Day 16 的 block 级和今天的 warp 级，关键差异在"结果广播"：

```cuda
// Day 16 block 级：结果只有 lane 0 有，必须经 shared memory 广播
...
# 对比：也 profile Day 16 的 block/scalar 版本
ncu --metrics \
  dram__throughput.avg.pct_of_peak_sustained_elapsed,\
  sm__throughput.avg.pct_of_peak_sustained_elapsed,\
  gpu__time_duration.sum \
...
> 💡 提交后在 [LeetGPU Argmax 题目](https://leetgpu.com/challenges/argmax)上记录通过耗时。完整题解（含平局处理图解、`atomicMax` vs 两次 kernel 的权衡）见 [Argmax 题解](../../leetgpu/leetgpu-argmax-solution.md)。
...
| 优化后比 Day 16 还慢 | M 太小，warp 级并行度不足 | 增大 M（如 1024+），让 warp 数足够多填满 SM |
...
- [ ] 完成 Day 16 版本的至少一项优化（warp 级或 float4），并用 ncu 验证 DRAM Throughput 提升
- [ ] 能对照昇腾解释 Vector Unit 与 CUDA float4/half2 的对应关系（天然向量化 vs 手动 cast）

---

### 今日总结

Day 17 我们读了 PyTorch 和 FasterTransformer 的官方源码，找到 Day 16 手写版的三个差距并动手优化：
...
6. **昇腾对照**：CUDA 的 float4/half2 手动 cast ↔ 昇腾 Vector Unit 天然向量化，本质都是"一次处理多元素"

掌握这些后，你就建立了"看到 memory-bound kernel 就条件反射想到向量化 + reduce 合并 + fusion"的工程直觉。Day 18 会把这套分析框架用到标准 Attention 的 IO 复杂度上。
...
3. **什么是向量化加载？为什么 float4 能让 memory-bound kernel 提速 2-4x？**
   ...
   - **昇腾对照**：昇腾 Vector Unit 指令天然向量化，无需手动 `reinterpret_cast<float4*>`
