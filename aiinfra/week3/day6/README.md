## Day 20：端到端 Profiling 与 Kernel Fusion
...
6. 能对照昇腾 **msprof** 解释 nsys/ncu 指标的映射关系（AI Core Utilization、Memory Bandwidth Utilization）

> 💡 **为什么重要**：Day 15-19 我们分别手写了 Softmax/LayerNorm/Attention、接入 Mini Engine。但"算子各自正确"不等于"系统跑得快"——真实优化必须先**定位瓶颈**再动手。今天就是把"会写 kernel"升级为"会用工具诊断系统"的关键一天，五步 Profiling 法是所有 GPU 性能优化的标准工作流。Day 21 会把今天的结论整理成算子分类表。

---

### 学前导读：从"单算子正确"到"系统级瓶颈定位"

Day 19 的 Mini Engine 跑通了：自定义 Softmax/LayerNorm + cuBLAS GEMM，端到端误差 < 1e-4。但你可能注意到一个尴尬的现象——**自定义版比 PyTorch 还慢一点（0.8x ~ 0.95x）**。问题来了：慢在哪里？是 Softmax 拖后腿，还是 GEMM 没跑满，还是 kernel 之间的空隙太大？

回答这个问题，靠"猜"是不行的。Day 15 我们用过 `torch.profiler` 看算子时间表，但它只告诉你"哪个算子慢"，不告诉你"为什么慢"——是算力不够，还是带宽喂不饱？要回答"为什么"，需要更底层的工具：
...
> ⚠️ **注意**：`torch.compile` 的 fusion 作用于 PyTorch 原生 ATen 算子。**自定义 C++ Extension 算子对 Inductor 是"黑盒"**，无法被融合（除非注册为 custom op + 提供 fake tensor）。这是 Day 19 自定义算子的一大代价——也是为什么"能不自定义就不自定义，先用 torch.compile"。
...
#### 20.4 昇腾对照：msprof 指标映射

五步法诊断逻辑跨平台通用，只是工具和指标名不同：

| ncu / nsys 指标 | 昇腾 msprof 对应 | 对照说明 |
|---------|------------|------|
| nsys 系统级时间线 | msprof timeline | 两者都提供算子级时间线 + kernel 排列 |
| ncu SM Throughput | AI Core Utilization | 计算单元利用率，含义一致 |
| ncu DRAM Throughput | Memory Bandwidth Utilization | 显存带宽利用率，含义一致 |
| ncu Long Scoreboard Stall | Memory Access Stall | 内存访问等待，跨平台一致 |
| ncu Math Pipe Throttle | Compute Pipe Stall | 计算单元饱和 |
| `--kernel-name regex:` 过滤 | Operator 详情面板 | 都能定位到单个算子 |

**关键差异**：CUDA 把"系统级时间线"和"kernel 级指标"分散到 **nsys + ncu 两个工具**；昇腾 **msprof 把两层信息合并到一个 timeline**（既有算子时间，也有 AI Core / Memory 利用率）。迁移到昇腾只需把指标名换成 msprof 的对应项，诊断逻辑原样复用。
...
// [Memory-bound] Softmax：一行一个 block，三遍扫描 safe softmax（复用 Day 16）
...
2. 复用 Day 16 的 `warpReduceSum` + `blockReduceSum`（只 reduce 一次，比 LayerNorm 简单）
...
> 💡 提交后在 [LeetGPU RMS Normalization 题目](https://leetgpu.com/challenges/rms-normalization)上记录通过耗时，用 ncu 验证 `DRAM% >> SM%`（memory-bound），并对比 RMSNorm（一次 reduce）vs Day 16 LayerNorm（两次 reduce）的 latency。完整题解（含 RMSNorm vs LayerNorm 对比、Roofline 分析、与 Llama 的关联）见 [RMS Normalization 题解](../../leetgpu/leetgpu-rms-normalization-solution.md)。
...
> 提示：Prefill 的 top3 通常是 GEMM（compute-bound → Tensor Core）；Decode 的 top3 可能包含 LayerNorm/Softmax（memory-bound → fusion），且 launch overhead 占比更高 → CUDA Graph。这正是 Day 21 算子分类表的核心内容。

---

### 今日总结

Day 20 我们用 nsys + ncu 对 Mini Engine 做了端到端 Profiling，建立了"五步诊断法"：
...
6. **昇腾对照**：msprof 把 nsys+ncu 两层信息合并到一个 timeline，五步法诊断逻辑跨平台通用

掌握五步法后，任何 GPU 程序的瓶颈诊断都有章可循。Day 21 会把今天的结论整理成 Prefill/Decode 算子分类表，为 Week 4 FlashAttention 收尾。
...
2. **什么是 kernel fusion？为什么能提升性能？举一个 Transformer 中的例子。**
   ...
   - **昇腾对照**：CANN 算子库已内置大量融合算子（如 LayerNorm+Matmul）；PyTorch 2.0 `torch.compile` 自动融合 ATen 算子
...
4. **ncu 和 nsys 有什么区别？分别什么场景用？**
   ...
   - **昇腾对照**：昇腾 msprof 把两层信息合并到一个 timeline（既有算子时间也有 AI Core/Memory 利用率）
...
5. **为什么 `torch.compile` 能减少 kernel 数量？它对自定义 C++ Extension 算子有效吗？**
   ...
   - **对自定义 C++ Extension 无效**：自定义算子对 Inductor 是"黑盒"——它不知道算子内部逻辑，无法融合。这是 Day 19 自定义算子的代价之一
