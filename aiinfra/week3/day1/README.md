...
5. 能对照昇腾 CANN 的 `msprof` 解释 PyTorch profiler 的对应关系
...
![PyTorch profiler vs 昇腾 msprof](../website/images/profiler_comparison.svg)
...
#### CUDA/PyTorch → 昇腾 CANN 概念映射

| CUDA/PyTorch 概念 | 昇腾 CANN 对应 | 对照说明 |
|---------|------------|---------|
| Prefill 阶段（大 GEMM） | Prefill 阶段（Cube Core 大矩阵乘） | 两阶段划分是模型层面的，与硬件无关 |
| Decode 阶段（小 GEMM + KV Cache） | Decode 阶段（Vector + Cache 读取） | 昇腾 Vector Unit 利用率低，与 SM 空闲同理 |
| torch.profiler 时间线 | msprof timeline | 两者都提供算子级时间线 |
| `F.softmax` / `F.layer_norm` | Ascend C 内置 Softmax/LayerNorm 算子 | 语义一致，PyTorch 调 CUDA kernel，CANN 调 NPU 算子 |
| Attention（QK^T + softmax + PV） | FlashAttention 算子（CANN 内置） | 昇腾 CANN 已内置 FlashAttention |
