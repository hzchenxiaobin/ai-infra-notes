...
6. 能对照昇腾 CANN 的 L0 Buffer 解释 shared memory tile 驻留与 warp 分工的对应关系
...
创建文件 [kernels/flash_attention_v2.cu](kernels/flash_attention_v2.cu)：
...
```bash
# 编译
nvcc -o flash_attention_v2 kernels/flash_attention_v2.cu -O3 -arch=sm_80
...
nvcc -Xptxas -v -o flash_attention_v2 kernels/flash_attention_v2.cu -O3 -arch=sm_80
...
```bash
nvcc -o flash_attention_v2 kernels/flash_attention_v2.cu -O3 -arch=sm_80 -g -lineinfo
...
本题核心是 **block 归约 + kernel 融合**——把 element-wise 乘法与归约合并到单 kernel，避免中间数组。FlashAttention Kernel 中的 `warpReduceSum` / `warpReduceMax` 原语正是从这类归约问题抽象出来的。今天手写 FA 时复用了 Week 2 Day 1 的归约原语，本题反向巩固这些原语的最基本应用。

**解题思路**：

两阶段归约：① grid-stride 算 `a[i]*b[i]` 的局部和（融合乘法与累加，无中间数组）② 两级 block 归约（warp shuffle + shared memory 中转 + warp 0 最终归约）③ 第二遍 kernel 归约各 block 部分和。

> 💡 提交后在 [LeetGPU Dot Product 题目](https://leetgpu.com/challenges/dot-product)上记录通过耗时。完整题解（含 kernel 融合、float4 向量化、cooperative_groups 优化）见 [Dot Product 题解](../../leetgpu/week4/day2/leetgpu-dot-product-solution.md)。
...
> 提示：SRAM = (Br + 2×Bc) × d × 4 bytes。Br=Bc=128 时为 96 KB，仍在 A100 上限内但 occupancy 可能下降。
...
3. **FlashAttention Kernel 的 register 使用量为什么很大？会导致什么问题？**
   ...
   - 问题：register 压力大 → occupancy 下降（A100 每 SM 最多 65536 个 register，120 reg/thread 时每 SM 只能跑 ~544 线程 ≈ 2 个 block）
...
4. **SRAM 使用量如何计算？Br/Bc 选太大或太小有什么问题？**
   ...
   - 太大：超过 shared memory 上限（A100 164 KB/SM）→ 编译失败；或 occupancy 暴跌
...
6. **能对照昇腾解释 FlashAttention Kernel 的 shared memory 与 warp 分工吗？**

   - CUDA shared memory ≈ 昇腾 L0 Buffer / UB，都用于存储 tile 数据供计算单元共享
   - CUDA warp（32 线程）≈ 昇腾 Vector Group，都是片上最小的并行计算单元
   - CUDA `__shfl` warp 内通信 ≈ 昇腾 `__reduce_add` / `__reduce_max` 内置函数，都是寄存器级数据交换
   - 差异：CUDA 需手动管理 shared memory 加载和 `__syncthreads`；昇腾 Fixpipe 硬件自动完成 L1→L0 数据预取
