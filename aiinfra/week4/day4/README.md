...
6. 能对照昇腾 CANN 解释 FA2 的 warp group 优化在昇腾上如何对应
...
给定长度 N 的输入信号 `input` 和长度 K 的卷积核 `kernel`，计算 1D 卷积输出 `output[i] = Σ input[i+k-K/2..i+k+K/2] * kernel[k]`。约束 `1 ≤ N ≤ 1,000,000`，`1 ≤ K ≤ 15`（K 为奇数），元素范围 `[-1.0, 1.0]`。

**与今日知识的关联**：

本题考察 **shared memory halo**——每个 block 加载一段输入含 halo（边界），在 shared memory 中完成卷积。这与 FA2 的 **tile 管理思路**直接呼应：FA2 把 Q tile 划分子块给 warp groups，每个 group 管自己的子块边界；1D 卷积的 halo 管理也是"每个 block 管自己的一段+边界"。两者都是**分块时正确处理边界数据**的工程模式。

**解题思路**：

一个 block 处理一段输出 tile，协作加载含 halo 的输入到 shared memory（halo = K/2 个边界元素），kernel 权重放 `__constant__` memory 广播，每线程算若干输出元素。

> 💡 提交后在 [LeetGPU 1D Convolution 题目](https://leetgpu.com/challenges/1d-convolution)上记录通过耗时。完整题解（含 shared memory halo、`__constant__` 广播、边界处理）见 [1D Convolution 题解](../../leetgpu/week4/day4/leetgpu-1d-convolution-solution.md)。
...
5. **能对照昇腾解释 FA2 的 warp group 优化吗？**

   - CUDA 的 warp group 子块划分对应昇腾的 **Vector Group / Cube Group** 分组计算
   - CUDA 减少 non-matmul FLOPs ≈ 昇腾让 Vector Unit 独立处理 softmax（Vector Unit 天然向量化，non-matmul 效率更高）
   - CUDA 的 seq 并行 ≈ 昇腾 Cube Core 支持的序列分块并行
   - CUDA 更高 occupancy ≈ 昇腾更多 block 驻留 AI Core
   - FA2 反向传播 ≈ 昇腾 CANN 已内置完整 FA 算子（前向+反向）
   - 核心一致：都是"减少跨组同步，让计算单元自治"

6. **如果让你继续优化 FlashAttention（FA3 方向），你会怎么做？**
