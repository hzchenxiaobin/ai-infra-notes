...
★ = 自定义 CUDA FlashAttention kernel（Day 2 实现）
```
...
6. **能对照昇腾解释自定义算子集成的方式吗？**

   - CUDA 的 PyTorch C++ Extension 对应昇腾的 **Ascend C 自定义算子 + PyTorch NPU**
   - CUDA 的 `load_inline` 动态编译对应昇腾的 `aclOpCompile` + 动态加载
   - CUDA 的 `torch.mm`（cuBLAS）对应昇腾的 `aclnnMatmul`（ACL MatMul）
   - 自定义 FlashAttention 在 CUDA 是学习目的；昇腾 CANN 已内置完整 FA 算子，生产环境直接调用
