...
// 编译命令: nvcc -o dot_product dot_product.cu -O3 -arch=sm_80
...
__global__ void dot_kernel(const float* a, const float* b, float* block_sums, int N) {
    ...
    if (wid == 0) {
        ...
        if (lane == 0) block_sums[blockIdx.x] = local_sum;
    }
}
...
int main() {
    ...
    free(h_a); free(h_b); cudaFree(d_a); cudaFree(d_b); cudaFree(d_block); cudaFree(d_out);
    return 0;
}
```
