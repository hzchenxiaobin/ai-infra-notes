...
创建文件 [kernels/benchmark_flash_attention.py](kernels/benchmark_flash_attention.py)：
...
> 提示：bandwidth = HBM_IO / latency；FLOPS = 2·B·H·N²·d / latency。对比是否接近 A100 峰值（1.55 TB/s 带宽，19.5 TFLOPS FP32）。
...
6. **能对照昇腾解释 ncu 的 HBM 验证方法吗？**

   - CUDA 的 `ncu --metrics dram__bytes_read/write` 对应昇腾 **msprof** 的 Memory Bandwidth Utilization 指标
   - 两者都测量 kernel 的实际 HBM 读写量，用于验证 IO 复杂度
   - 验证逻辑一致：N 翻倍时 IO 翻倍 → O(N)；N 翻倍时 IO 4x → O(N²)
   - 差异：ncu 是 kernel 级深度分析工具，msprof 集成在昇腾开发环境中，提供 AI Core + Memory 综合视图
