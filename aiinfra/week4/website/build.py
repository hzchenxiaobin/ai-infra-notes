#!/usr/bin/env python3
"""
Build the Week 4 website from README.md (overview) and dayN/README.md (per-day).
Generates:
  - index.html: overview page (from week4/README.md)
  - dayN.html: one page per day (from week4/dayN/README.md)
Uses relative paths (../css/..., ../js/...) for shared resources,
since week4/ is one level below the deployment root on GitHub Pages.
"""
...
def load_overview_and_days():
    """Load overview from week4/README.md and per-day markdown from week4/dayN/README.md."""

    readme_path = WEEK4_DIR / "README.md"
    if not readme_path.exists():
        raise FileNotFoundError(f"Week 4 README not found: {readme_path}")
    overview = readme_path.read_text(encoding="utf-8").replace("](website/images/", "](images/")

    day_title_pattern = re.compile(r"^## Day (\d+)[：:]\s*(.+)$")
    days = []
    for day_dir in sorted(WEEK4_DIR.glob("day*")):
    ...
    if not days:
        raise ValueError(f"No day*/README.md files found in {WEEK4_DIR}")
...
def build_nav(current_day: Optional[int] = None, weeks: Optional[list] = None,
    ...
    lines.append(f'<a class="{overview_class}" href="index.html">📌 Week 4 概览</a>')

    lines.append('<div class="nav-section-title">8 周学习路线</div>')

    # Titles for all weeks; week 4 title is hardcoded, others come from the plan.
    week_titles = {4: "FlashAttention 深挖"}
    for week in weeks:
        week_titles[week["num"]] = week["title"]

    repo_root = WEEK4_DIR.parent
    existing_days = get_day_numbers(WEEK4_DIR)
    week_data = [
        {
            "num": 1,
            "href": "../index.html",
...
def page_template(title: str, nav_html: str, markdown: str,
    ...
    page_title = page_title if page_title is not None else f"Week 4 - {title}"
...
def build_website(output_dir: Path) -> None:
    ...
    overview_html = page_template(
        ...
        is_overview=True,
        page_title="Week 4 - Transformer 执行本质与算子手写",
    )
