...
| **GEMM 优化层次** | "记得有 tiling，但记不住每层 %" | 画 8 层阶梯图，背理论阶梯（1%→15%→40%→55%→60%→70%→80%+→90%+）+ 实测阶梯（week10/day7：10.6%→13.3%→30.8%→64.3%→62.9%→63.8%） |
...
创建文件 [kernels/knowledge_selftest.py](https://github.com/hzchenxiaobin/ai-infra-notes/blob/main/aiinfra/daily/week10/day7/kernels/knowledge_selftest.py)，把六大薄弱点 + 关键公式 + RTX 5090 参数做成一个自测系统：
...
完整代码见 [kernels/knowledge_selftest.py](https://github.com/hzchenxiaobin/ai-infra-notes/blob/main/aiinfra/daily/week10/day7/kernels/knowledge_selftest.py)。
...
| GEMM 8 层优化 + 增益 % | 理论阶梯 1%→90%+ + 实测阶梯（week10/day7）10.6%→63.8% | < 3 min |
...
针对 Day 5 暴露的薄弱点和今天重学的内容，再做一轮 Mock 面试（用 `week10/day7/kernels/mock_interview.py`）。重点观察：
...
完成今天的查漏补缺后，你应该能对每个高频面试题在 3 秒内给出准确数字和公式。明天 Day 7 进入最终复盘，画 10 周能力地图、规划后续路线，完成整个学习闭环。
...
<summary>点击查看答案</summary>
 ...
 实测数据（week10/day7 · RTX 5090 · M=N=K=4096，cuBLAS 基线 68.2 TFLOPS = 100%）：
