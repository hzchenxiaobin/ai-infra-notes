...
- **RTX 5090**：164 KB/SM shared memory
...
1. **Tensor Core**：官方用 WMMA/mma.sync 做 QK^T 和 PV 的 GEMM，峰值 4-8x；手写版用 FMA 标量
...
- **标准 Attention**：latency 近似随 N² 增长（HBM IO 是 O(N²)，N 翻倍 IO 变 4x）
