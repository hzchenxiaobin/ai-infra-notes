...
标准 attention 的 backward 需要物化 `S = QK^T` 和 `P = softmax(S)` 两个 $N \times N$ 矩阵——因为 PyTorch autograd 默认把前向的中间张量存进计算图，反向时直接读取。以 N=4096, d=64, FP32 为例：
...
<summary>点击查看答案</summary>
 ...
 - 标准 attention 的 backward 公式需要 `P = softmax(S)`（$N \times N$），而 FA Forward 为了省内存**根本没存 P**——它用 online softmax 在 SRAM 里算完就丢弃了
