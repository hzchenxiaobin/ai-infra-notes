...
**注意力变体（一般形式）**：把 `n_kv_head` 换成变体实际值即可——GQA-8（如 LLaMA-3-8B）降为 1/4，MQA 降为 1/n_head，MLA 压缩到低秩维度 d_c。变体对比表见 `week10/day3` §2.2。
...
来源：[实测/手算] 与 `week10/day3`、`week10/day5` 教程口径一致；公式本身为教科书定义。

## 3. GEMM 优化增益（唯一口径：week10/day1 实测）

来源：[实测] `week10/day1/README.md`（RTX 5090，sm_120，CUDA 12.8，`kernels/gemm_optimization_series.cu`，FP32，cuBLAS 基线）。**面试统一背这一套**：
...
**理论分层口径**（week10/day1 面试要点的叙事版，用于"每一层收益来源"的口述框架，**与实测数字并存使用、不得混为一谈**）：SharedMem 1%→15% → RegBlk →45% → float4 →55% → Warp Shuffle →60% → Double Buffering →70% → Auto-tune →80% → Tensor Core/指令级 →90%+。说这套时必须声明是"经验分层估计"，实测锚点用上面的表。
...
**N=4096, d=64, FP32 代值**（来源：`week9/day6` 推导 + `week10/day1` 实跑脚本理论值输出）：
...
>
> ⚠️ week9/day6 现有 "~100x / ~206MB vs 2MB" 表格与本口径不一致，属待收敛项（A2 任务处理），正文写作以本节为准。
