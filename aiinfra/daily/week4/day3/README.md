...
**核心洞察**：Welford 算法用递推公式 $\mu_n = \mu_{n-1} + \frac{x_n - \mu_{n-1}}{n}$ 在一次遍历中同时更新 mean 和 variance，消除了"先求 μ 再求 σ²"的串行依赖。
...
1. **Welford 算法**：递推公式 $\text{mean} += \delta/\text{count};\ M2 += \delta \cdot \delta_2$ 在一次遍历中同时算 mean 和 variance，HBM 读写从 4×2D 降到 2×2D
