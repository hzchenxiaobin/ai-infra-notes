...
// ncu 分析: ncu --metrics
// dram__throughput.avg.pct_of_peak_sustained_elapsed,sm__throughput.avg.pct_of_peak_sustained_elapsed --kernel-name
// regex:"softmax_kernel|gemm_kernel" ./profiling_targets
...
int main() {
    ...
    float *h_sm_in = (float*)malloc(sm_bytes), *h_sm_out = (float*)malloc(sm_bytes),
          *h_sm_ref = (float*)malloc(sm_bytes);
    initData(h_sm_in, SM_M * SM_D);
    float *d_sm_in, *d_sm_out;
    cudaMalloc(&d_sm_in, sm_bytes);
    cudaMalloc(&d_sm_out, sm_bytes);
    ...
    initData(h_A, G_M * G_K);
    initData(h_B, G_K * G_N);
    float *d_A, *d_B, *d_C;
