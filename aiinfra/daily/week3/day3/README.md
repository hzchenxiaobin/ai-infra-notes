...
A fragment 是 16×16 矩阵，由 32 线程持有，每个线程持有 8 个 FP16 元素（4 个寄存器，每寄存器 2 个 FP16）：

```
线程编号: 0-3   → A[0:4, 0:8]   和 A[0:4, 8:16]
线程编号: 4-7   → A[4:8, 0:8]   和 A[4:8, 8:16]
线程编号: 8-11  → A[8:12, 0:8]  和 A[8:12, 8:16]
线程编号: 12-15 → A[12:16, 0:8] 和 A[12:16, 8:16]
线程编号: 16-31 → 重复 0-15 的映射（另一半）
```

| 线程 | 寄存器 a0 | 寄存器 a1 | 寄存器 a2 | 寄存器 a3 |
|------|----------|----------|----------|----------|
| t0 | A[0,0],A[0,1] | A[0,2],A[0,3] | A[0,8],A[0,9] | A[0,10],A[0,11] |
| t1 | A[1,0],A[1,1] | A[1,2],A[1,3] | A[1,8],A[1,9] | A[1,10],A[1,11] |
| ... | | | | |

> ⚠️ **这个映射是硬件固定的**。`ldmatrix` 指令的存在就是为了自动完成这个映射——你给它 shared memory 地址，它按这个布局把数据加载到正确的寄存器。
