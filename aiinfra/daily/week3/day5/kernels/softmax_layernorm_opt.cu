...
// ============================================================
__global__ void layernorm_scalar_kernel(const float* __restrict__ input, const float* __restrict__ gamma,
                                        const float* __restrict__ beta, float* __restrict__ output, int M, int N,
                                        float eps) {
    int row = blockIdx.x;
    if (row >= M)
...
int main() {
    ...
    // ---- Softmax 对比 ----
    printf("[Softmax: block-level (week9/day6) vs warp-level (optimized)]\n");
    ...
    printf("  block-level (week9/day6): %.4f ms\n", ms_block);
    printf("  warp-level (optim) : %.4f ms\n", ms_warp);
    printf("  speedup            : %.2fx\n\n", ms_block / ms_warp);

    // ---- LayerNorm 对比 ----
    printf("[LayerNorm: scalar load (week9/day6) vs float4 vectorized vs Welford one-pass]\n");
    ...
    printf("  scalar (week9/day6) : %.4f ms\n", ms_scalar);
