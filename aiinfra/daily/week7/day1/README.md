...
创建文件 [kernels/continuous_batcher.py](https://github.com/hzchenxiaobin/ai-infra-notes/blob/main/aiinfra/daily/week7/day1/kernels/continuous_batcher.py)，实现 iteration-level 调度：
...
=== Continuous vs Dynamic Batching Comparison ===
 Continuous: S3 (短请求) 在 iter 5 完成，立即退出，不等待 S2
 Dynamic: S1,S2,S3 同 batch 开始，S3 完成后要等 S2（到 iter 8）
...
3. **S3 在 iter 5 完成**：短请求（gen=3）先于 S2（gen=8）完成，**不等 S2**
4. **batch size 动态变化**：2→2→3→3→2→1→1→1（完成退出、新请求加入）
5. **对比 Dynamic**：S3 节省 3 个 iteration 的等待（Dynamic 下 S3 要等 S2 到 iter 8）
...
7. **手写 ContinuousBatcher**：实测 S3（短请求）在 iter 5 完成，比 Dynamic 节省 3 个 iteration 等待
