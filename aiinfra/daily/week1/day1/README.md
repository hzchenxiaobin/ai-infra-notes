...
![GPU 硬件层次总览](../../images/week1_gpu_hierarchy.svg)
...
![Grid/Block/Warp/Thread 层次](../../images/week1_grid_block_hierarchy.svg)
...
![SIMT vs SIMD](../images/simt_vs_simd.svg)
...
![Warp Divergence](../images/warp_divergence.svg)
...
![Grid Block Thread 层次](../images/grid_block_thread.svg)
...
这些限制决定了单个 block 的大小不能无限大。例如，一个 block 有 1024 个线程，每个线程用 64 个寄存器，每个寄存器 4 字节，则寄存器总量为 1024 × 64 × 4B = 256 KB，刚好占满 SM 的 256 KB 寄存器文件，一个 SM 只能同时跑这一个 block；但如果每个线程用 256 个寄存器，则一个 block 就需要 1024 × 256 × 4B = 1024 KB = 1 MB，远超 256 KB 限制，无法启动。
...
![线程 ID 计算](../images/thread_id_calculation.svg)
...
```bash
# 编译
nvcc -o hello_gpu day1/kernels/hello_gpu.cu
...
![Block 到 Warp 的映射](../../images/week1_block_warp_mapping.svg)
