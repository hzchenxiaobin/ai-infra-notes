...
![Online Softmax 三公式推导](../website/images/online_softmax_derivation.svg)

这是 FlashAttention 的核心创新，也是面试必考的白板推导题。下面从标准 softmax 出发，一步步推导出三个增量更新公式。
...
对每一行（每个 query），维护三个量 $(m, l, o)$。设已经处理过的所有 score 组成集合 $X_{\text{old}}$，对应的 value 为 $v_x$：
...
- $o$：已归一化的部分输出（softmax 加权平均）

  $$
  o = \frac{\sum_{x \in X_{\text{old}}} \exp(x - m) \cdot v_x}{l}
  $$

**初始状态**：$m = -\infty,\; l = 0,\; o = \mathbf{0}$（零向量）。

##### 3. 新块到来时的修正原理

现在来了新块 $X_{\text{new}} = \{x_j\}$，对应的 value 为 $\{v_j\}$。
...
旧的 $l$、$o$ 都是在旧参考点 $m$ 下算的。现在参考点变成 $m_{\text{new}}$，需要把所有旧指数"换算"到新参考点：
