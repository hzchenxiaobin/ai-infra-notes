...
本周贯穿始终的一条暗线是「CUDA 与昇腾 CANN 的对照」。整理这张映射表，是跨平台迁移能力的核心。

| 维度 | CUDA 概念 | 昇腾 CANN 概念 | 迁移要点 |
|------|---------|--------------|---------|
| Warp 级通信 | `__shfl_down_sync` | `__reduce_add`（Ascend C 内置） | CUDA 需手写 butterfly 循环；昇腾调用高级 API |
| 寄存器累加 | `acc[TM][TN]` in register | L0 Buffer accumulator | CUDA 用寄存器文件；昇腾用 L0 Buffer SRAM |
| 片上共享 | `__shared__` | L1 Buffer (UB) | 功能等效，都是片上快速存储 |
| 双缓冲 | 手动声明两份 buffer | Fixpipe 自动流水线 | CUDA 需手动实现；昇腾硬件自动完成 |
| 异步流 | `cudaStreamCreate` | `aclrtCreateStream` | API 语义完全一致 |
| 性能分析 | Nsight Compute (ncu) | Ascend Profiler (msprof) | 功能对等，Nsight GUI 更成熟 |
| 矩阵分块 | BM×BN×BK thread tile | Cube Unit 分块 + FRACTAL_NZ | 昇腾 Cube Core 一次处理更大 tile |
| 专用矩阵单元 | Tensor Core (WMMA) | Cube Core | 都是专用矩阵乘累加单元 |

**跨平台迁移心法**：

1. **算法层一致**：分块、复用、流水线思想跨平台通用
2. **抽象层差异**：CUDA 暴露底层细节（需手写 shuffle），昇腾提供高级 API（`__reduce_add`）
3. **硬件层对应**：Shared Memory ↔ L1 Buffer，Tensor Core ↔ Cube Core，寄存器 ↔ L0 Buffer
4. **调优层复用**：Roofline 模型、Occupancy 分析、Memory Bound 判定方法完全通用

> 💡 **一句话总结**：跨平台迁移的本质是「算法思想不变，硬件抽象重映射」。掌握本周的 CUDA 优化层次，迁移到昇腾只需重新对应硬件单元，而非重新学习优化方法论。
...
4. **你的 GEMM Kernel 和 cuBLAS 的差距在哪里？要达到 90% 还需要做什么？**
   ...
   - **昇腾对照**：cuBLAS ≈ 昇腾的 ACL MatMul 算子库，手写 Kernel 达到 70% 已是非常优秀的水平

5. **跨平台迁移：CUDA 的 Warp Shuffle 到昇腾怎么对应？**

   - CUDA 的 `__shfl_down_sync` 需手动写 butterfly 循环；昇腾 Ascend C 提供 `__reduce_add` 等高级归约内置函数
   - CUDA 需显式指定 `0xFFFFFFFF` mask；昇腾 Vector Unit 默认全线程参与
   - 底层一致：都依赖片上最快的寄存器级数据交换通路，延迟量级相当（CUDA ~1-2 cycle，昇腾 ~2-4 cycle）
   - 迁移心法：算法思想（分块、复用、流水线）不变，只重映射硬件抽象（Shared Mem↔L1 Buffer、Tensor Core↔Cube Core）
