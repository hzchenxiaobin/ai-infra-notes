...
**公式 23（$\mathcal{L}_{ExpBal}$）的推导/设计动机**：
...
1. **$f_i$ 的归一化含义**：
   ...
   它的设计让"理想均匀负载"时 $f_i = 1$（每个专家平均负责 $K_r/N_r$ 的 token）。$f_i > 1$ 表示该专家超载，$f_i < 1$ 表示欠载。
...
2. **$P_i$ 是路由器的软输出**：
   ...
   $s_{i,t}$ 是 token $t$ 对专家 $i$ 的亲和力（softmax 后的概率），对路由参数 $e_i$ 和输入 $u_t$ 可导。

3. **为什么用乘积 $f_i P_i$**：

   我们希望**超载的专家被少选、欠载的专家可多选**。但 $f_i$ 是 Top-$K$ 硬选择的统计量，梯度无法直接传回去；$P_i$ 是 soft 的，可以求梯度。把二者相乘：

   $$
   \mathcal{L}_{ExpBal} = \alpha_1 \sum_i f_i P_i
   $$

   对某个 token 的专家亲和力 $s_{i,t}$ 求梯度（把 $f_i$ 看作常数）：

   $$
   \frac{\partial \mathcal{L}_{ExpBal}}{\partial s_{i,t}} \propto f_i
   $$

   这意味着：
   - 专家 $i$ 超载（$f_i > 1$）时，即使 $P_i$ 不算大，损失也会给它一个**更强的负梯度**，训练过程中路由器会更少把 token 分配给它；
   - 专家 $i$ 欠载（$f_i < 1$）时，惩罚变轻，允许其亲和力自然上升、吸引更多 token。

   直观上，$\sum_i f_i P_i$ 同时惩罚"负载高"和"亲和力高"的专家——因为 Top-$K$ 路由下，二者往往同时出现在少数"明星专家"身上，正是路由崩塌（routing collapse）的根源。

4. **与 Switch Transformer 损失的联系**：

   该 $f \cdot P$ 形式直接继承自 Switch Transformer / GShard 的专家级均衡损失。它等价于最小化负载分布与均匀分布之间的某种"加权 KL/散度"：当所有专家 $f_i \approx 1$ 时，损失达到最小值。
...
- MoE 前向：计算量 $\propto$ 8 个专家（2 共享 + 6 激活）而非 160；瓶颈是专家并行的 **all-to-all 通信**——设备受限路由（M=3）+ 三级均衡 + 通信/计算重叠专为压它而设。
...
- **注意力消融（Table 8/9，见 §8）**：全文最关键证据——MLA > MHA > GQA > MQA 的能力排序与 $MLA \approx MHA \times 4\%$ 的 KV 同时成立。
...
| GQA（Ainslie 2023） | 分组共享 K/V | 同上；$MLA \approx$ GQA-2.25 组的缓存但更强 | 可调档位、实现简单 | 仍有能力折损 |
...
- **结构层省 KV 与系统层省 KV 的互补**：MLA（结构压缩）与 vLLM PagedAttention（存储管理）、KV 量化（数值压缩）是三个正交层面——本文部署时三者叠加（MLA $57\times$、6bit 量化 $2.7\times$，$\approx$ 总 $-93.3\%$）。做 serving 时这条"乘法链"思维比单点优化重要。
