#!/usr/bin/env python3
"""
Build the Triton topic website from README.md (overview) and dayN.md (per-day).

Generates:
  - index.html: overview page from README.md
  - day1.html ~ dayN.html: one page per day
  - images/: copies Triton-related SVGs from aiinfra/topics/images/
  - kernels/: copies from aiinfra/topics/triton/kernels/

This builder is designed to be copied into public/triton/ by the root build.py,
...
TRITON_DIR = Path(__file__).parent.parent
TOPICS_DIR = TRITON_DIR.parent
...
def escape_for_template_string(text: str) -> str:
    ...
    text = text.replace("`", "\\`")
    text = text.replace("${", "\\${")
    text = text.replace("\x3c/script>", "\\x3c/script>")
    return text


def rewrite_local_paths(markdown_text: str) -> str:
    """Rewrite local asset paths so they work after deployment to public/triton/."""
    # Images referenced as ../images/xxx.svg from aiinfra/topics/triton/dayN.md
    # should become images/xxx.svg relative to public/triton/dayN.html.
    markdown_text = re.sub(r"\]\(\.\./images/", "](images/", markdown_text)
    return markdown_text


def extract_days() -> list:
    """Extract day info from triton/dayN.md files.

    Returns list of {"num": int, "title": str, "markdown": str} sorted by day number.
    """
    day_title_pattern = re.compile(r"^# Day (\d+)[：:]\s*(.+)$")
    days = []
    for md_path in sorted(TRITON_DIR.glob("day*.md")):
...
def build_nav(current_day: Optional[int] = None) -> str:
    """Build sidebar navigation for the Triton topic site."""
    lines = []
    ...
    lines.append('<div class="nav-section-title" style="margin-top:1rem;">Triton 专题</div>')
...
def page_template(
    ...
    page_title = page_title if page_title is not None else f"Triton - {title}"
...
def copy_images(output_dir: Path) -> None:
    """Copy Triton-related SVG images from aiinfra/topics/images/ to output/images/."""
    ...
    for item in src.iterdir():
        if item.is_file() and item.suffix.lower() == ".svg" and item.name.startswith("triton_"):
            shutil.copy2(item, dst / item.name)
            copied += 1
    print(f"Copied {copied} Triton SVG images: {src} -> {dst}")


def copy_kernels(output_dir: Path) -> None:
    """Copy Triton kernel sources to output/kernels/."""
    src = TRITON_DIR / "kernels"
...
def build_website(output_dir: Path) -> None:
    ...
    days = extract_days()
    if not days:
        raise ValueError(f"No day*.md files found in {TRITON_DIR}")

    # Overview page from README.md
    readme_path = TRITON_DIR / "README.md"
    if not readme_path.exists():
        raise FileNotFoundError(f"Triton README not found: {readme_path}")
    ...
    overview_html = page_template(
        ...
        is_overview=True,
        page_title="Triton 专题 - AI Infra 学习笔记",
    )
    ...
    for day in days:
        html = page_template(
            ...
            is_overview=False,
            page_title=f"Triton Day {day['num']} - {day['title']}",
        )
