...
6. 能对照昇腾 CANN 解释 KV Cache 的概念一致性，说出 3 个 KV Cache 优化方向（PagedAttention、量化、GQA）
...
| GQA（少 KV 头） | GQA | KV head 复用，跨平台一致 |
| 多轮对话复用 | 历史 Cache 复用 | 策略一致 |

> 💡 KV Cache 是**模型层面**的数据结构，与硬件无关——无论 CUDA 还是昇腾，都是"把 K/V 存下来避免重算"。差异只在底层 memory allocator（CUDA `cudaMalloc` vs 昇腾 `aclrtMalloc`）和 PagedAttention 的具体实现细节。
