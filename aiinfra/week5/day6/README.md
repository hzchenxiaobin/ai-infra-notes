...
6. 能对照昇腾 `msprof` 解释 nsys/ncu 的指标对应关系，说清推理 profiling 跨平台的一致性
...
![推理 Profiling 三层方法论：系统级 → 阶段级 → Kernel 级](../website/images/profiling_three_layers.svg)
...
2. cuda.Event 测 TTFT/TBT → 发现 "TTFT 随 N² 增长，TBT 稳定"
   → 判断：Prefill compute-bound，Decode launch-bound

3. ncu 钻进最慢 kernel → 发现 "dram__throughput 95%，sm__throughput 5%"
...
![Decode 单步 Breakdown：forward / sampling / sync 占比](../website/images/decode_breakdown.svg)
...
算术强度 `AI = FLOPs / Bytes`，对照 Ridge Point（A100 ≈ 12.6 FLOP/Byte）：
- `AI < Ridge` → memory-bound（在带宽斜线上）
- `AI > Ridge` → compute-bound（在算力水平线上）

#### 6.5 瓶颈定位决策树

![瓶颈定位决策树：从现象到优化方向](../website/images/bottleneck_decision_tree.svg)
...
| dram__bytes | HBM 访问量 | 数据搬运量 |

> 💡 profiling 三层方法论跨平台通用——nsys 对应昇腾 msprof 的系统级时间线，ncu 对应 msprof 的算子级指标。指标名称不同但语义一致：都是"测时间、拆阶段、看带宽/算力利用率"。差异只在工具命令和具体指标命名。
...
4. **如何判断一个 kernel 是 compute-bound 还是 memory-bound？**
   ...
   - 或用 Roofline：算 `AI = FLOPs / Bytes`，对照 Ridge Point（A100 ≈ 12.6 FLOP/Byte）：AI < Ridge → memory-bound，AI > Ridge → compute-bound
...
6. **能对照昇腾 msprof 解释 nsys/ncu 的指标对应吗？**

   - 三层方法论跨平台通用：nsys 对应 msprof 系统级时间线，ncu 对应 msprof 算子级指标
   - 指标对应：TTFT↔首token时延、TBT↔逐token时延、kernel gap↔kernel间隙、SM Util↔AI Core利用率、Memory Throughput↔Memory Bandwidth
   - 语义一致：都是"测时间、拆阶段、看带宽/算力利用率"，判据（memory vs compute-bound）相同
   - 差异只在工具命令和指标命名；昇腾用 `msprof`，CUDA 用 `nsys`/`ncu`
