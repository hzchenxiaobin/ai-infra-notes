...
CUDA C Programming Guide 第 5 章（Performance Guidelines）是 CUDA 性能优化的核心章节。它不像前面章节那样介绍“怎么用 CUDA 编程”，而是告诉你“怎样才能把 CUDA 程序写得更快”。本章的建议来自 NVIDIA 官方多年工程实践，覆盖了从主机到设备的数据搬移、全局/共享/局部/常量/纹理内存的访问模式、Warp 级并发、Occupancy、指令吞吐，以及现代 GPU 上的一些高级特性（如异步拷贝、`__launch_bounds__`、TMA、DPX 等）。

建议阅读顺序：先通读一遍建立框架，然后在 Day 4–Day 6 遇到具体性能问题时回来查对应小节。本章内容量很大，不需要一次性全部记住，但至少要熟悉“性能调优的三根支柱”：**利用率（Utilization）**、**内存吞吐（Memory Throughput）**、**指令吞吐（Instruction Throughput）**。

---

##### 5.0 性能优化的整体思路

CUDA 程序的性能通常受限于以下三类因素之一：

1. **并行度不足**：GPU 没有足够多的活跃 Warp 来隐藏延迟（Latency Hiding）。
2. **内存带宽不足**：数据搬运或内存访问成为瓶颈。
3. **指令吞吐不足**：计算指令本身太慢，或使用了代价高的特殊函数、分支发散。

调优时可以用 Roofline 模型来帮助定位：先算出当前 kernel 的 **算术强度**（Arithmetic Intensity，每字节内存访问对应多少 FLOP），再对比 GPU 的峰值算力和峰值带宽，判断是 memory-bound 还是 compute-bound。

```
Arithmetic Intensity = FLOPs / Bytes
```

- 如果 AI 低、落在带宽墙以下 → 优化内存访问（coalescing、shared memory、减少搬运）。
- 如果 AI 高、落在算力墙以下 → 优化计算（指令选择、Occupancy、ILP）。

---

##### 5.1 Maximize Utilization（最大化利用率）

###### 5.1.1 Application Level：让 CPU 和 GPU 并行工作

CPU（Host）和 GPU（Device）是两套独立的计算资源。理想情况下：

- CPU 负责数据准备、I/O、调度。
- GPU 负责并行计算。
- 二者通过异步 CUDA Stream 重叠执行。

常用手段：

- **异步内存拷贝**：使用 `cudaMemcpyAsync` + page-locked（pinned）host memory，而非同步 `cudaMemcpy`。
- **多 Stream**：把任务拆成多个独立 Stream，让拷贝和计算重叠。
- **CUDA Graphs**：对重复执行的 kernel 序列录制一次、多次发射，降低 CPU launch overhead。
...
> **注意**：如果 host 内存不是 pinned，`cudaMemcpyAsync` 会退化为同步拷贝。

###### 5.1.2 Device Level：多 GPU 与 Multi-Process Service

- **多 GPU**：单节点多卡时，可用 `cudaSetDevice()` 选择设备，或在 NCCL 帮助下做数据并行。
- **MPS（Multi-Process Service）**：多个进程共享同一块 GPU 的上下文，减少上下文切换开销，提高小 kernel 的利用率。

###### 5.1.3 Multiprocessor Level：Occupancy 与延迟隐藏

SM（Streaming Multiprocessor）上的 warp scheduler 会在每个 cycle 选择“可执行”的 Warp 来执行。如果所有 Warp 都在等内存返回，SM 就会空闲。**高 Occupancy 能增加可选 Warp 数量，从而隐藏延迟**。

影响 Occupancy 的因素：

| 资源 | 说明 | 限制来源 |
|------|------|---------|
| 每个 block 的线程数 | block 越大，单个 SM 能同时驻留的 block 越少 | `maxThreadsPerBlock` |
| 每个线程的寄存器数 | 寄存器越多，SM 上能驻留的线程越少 | `regsPerBlock` / `regsPerMultiprocessor` |
| Shared memory | 占用越多，能并存的 block 越少 | `sharedMemPerBlock` |
| Block 总数 | SM 数量有限 | `multiProcessorCount` |

**关键指标**：

- **Occupancy** = 活跃 Warp 数 / SM 最大 Warp 数。
- **指令级并行（ILP）**：单个线程内部独立的指令越多，即使 Occupancy 不高也能隐藏延迟。
- **线程级并行（TLP）**：通过更多线程/Warp 来隐藏延迟。

官方建议：不要一味追求 100% Occupancy。很多时候 **75% 左右的 Occupancy 已经能获得接近峰值的性能**，因为一旦内存延迟被隐藏，继续增加 Warp 数量收益递减。真正的瓶颈可能是内存带宽或指令吞吐。

**实用工具**：

- 使用 `cudaOccupancyMaxPotentialBlockSize` 自动选择最佳 block size。
- 使用 Nsight Compute 的 *Occupancy* 章节查看实际瓶颈。
- 如果寄存器是瓶颈，可以尝试用 `__launch_bounds__(maxThreadsPerBlock, minBlocksPerMultiprocessor)` 提示编译器减少寄存器使用。
...
##### 5.2 Maximize Memory Throughput（最大化内存吞吐）

GPU 的显存带宽远高于计算延迟，但 misuse 会让有效带宽大打折扣。本章把内存访问分成两大类：

1. Host ↔ Device 数据传输。
2. Device 内部的各种内存访问（global / shared / local / constant / texture）。

###### 5.2.1 Host ↔ Device 数据传输

Host 与 Device 之间通过 PCIe / NVLink 连接，带宽远低于显存带宽。优化原则：**减少传输量、重叠传输与计算**。

- **Zero Copy**：在支持统一虚拟寻址（UVA）的系统上，可以让 kernel 直接访问 pinned host memory，适合只读或低频访问，但不要用于大规模读写。
- **Unified Memory（UM）**：由系统自动在 host/device 间迁移页，简化编程，但需要理解 page fault 和预取（`cudaMemPrefetchAsync`）才能用好。
- **Pinned Memory**：`cudaMallocHost` 分配，是异步拷贝和 Zero Copy 的前提。
- **数据本地化**：尽量让数据在 GPU 上分配、计算、消费，减少来回搬运。
...
###### 5.2.2 Global Memory：Coalescing 与对齐

Global memory 访问由 Warp 内 32 个线程共同发起。现代 GPU（Compute Capability ≥ 3.5）使用 **L1/Tex Cache** 和 **L2 Cache**，合并访问的原则是：

- Warp 内线程访问的地址落在同一个 cache line（通常 32 字节或 128 字节）内时，只需一次或少数几次事务。
- 访问越分散，事务越多，有效带宽越低。

**理想模式（行主序、连续访问）**：

```cuda
// 理想：threadIdx.x 访问连续的 float
globalMem[idx * blockDim.x + threadIdx.x]
```

**转置矩阵等 strided 访问**：

```cuda
// 不佳：threadIdx.x 访问列，导致跨步很大
globalMem[threadIdx.x * N + idx]
```

对于跨步访问，常用技巧是：

1. 先用 coalesced 读入 shared memory。
2. 在 shared memory 中做需要跨步访问的计算（注意 bank conflict）。

**对齐**：起始地址最好按 256 字节对齐（使用 `cudaMallocPitch` 分配 2D 数组），避免首尾 transaction 浪费。

```cuda
size_t pitch;
cudaMallocPitch(&d_ptr, &pitch, width * sizeof(float), height);
// 元素访问：d_ptr[row * pitch / sizeof(float) + col]
```

###### 5.2.3 Shared Memory：快但需管理 Bank Conflict

Shared memory 位于 SM 内部，延迟约为 global memory 的 1/100，带宽接近寄存器。它是实现 tiling、转置、归约、卷积等算法的关键。

- 被划分为 **32 个 bank**（Compute Capability ≥ 3.0 通常每个 bank 4 字节）。
- 同一 Warp 中多个线程访问**不同 bank** → 无 conflict，一次完成。
- 同一 Warp 中多个线程访问**同一 bank 的不同地址** → bank conflict，串行化访问。
- 同一 Warp 中所有线程访问**同一地址** → broadcast，无 conflict。

**避免 bank conflict 的常见方法**：

- 在 shared memory 数组的列维度加 padding。

```cuda
__shared__ float tile[TILE_DIM][TILE_DIM + 1];  // +1 消除转置时的 bank conflict
```

- 使用 warp shuffle 替代 shared memory 做 warp 内数据交换（减少 shared memory 压力）。

###### 5.2.4 Local Memory

Local memory 不是真正的“局部内存”，而是**位于 global memory 上的自动溢出空间**。编译器在以下情况会把变量放到 local memory：

- 数组索引在运行时无法确定。
- 结构体过大。
- 寄存器 spill（寄存器不够用）。

Local memory 访问通常不合并，因为每个线程的 local memory 区域是分开的。应尽量避免。

###### 5.2.5 Constant Memory

Constant memory 是只读、缓存的（每个 SM 有 constant cache）。适合**所有线程访问同一地址**的场景，例如查表、滤波器系数。

```cuda
__constant__ float filter[KERNEL_SIZE];
cudaMemcpyToSymbol(filter, h_filter, sizeof(filter));
```

如果不同线程访问不同地址，constant memory 会串行化，反而更慢。

###### 5.2.6 Texture / Surface Memory

Texture memory 也是只读缓存，具有 **2D 空间局部性缓存**，适合图像处理中的 2D 邻域访问。现代 GPU 上，通过 CUDA Array 或 `__ldg()`（load-global）也能获得类似缓存效果。

```cuda
// 只读全局加载，可被 L1/Tex cache 缓存
float v = __ldg(&globalArray[idx]);
```

---

##### 5.3 Maximize Instruction Throughput（最大化指令吞吐）

###### 5.3.1 算术指令

- **整数运算**：32 位整数加法和位运算通常每 cycle 一条；32 位整数乘法较慢（约 4 cycle）；除法和取模非常慢，尽量用位运算或预计算倒数替代。
- **浮点运算**：FP32 通常是 GPU 峰值算力的计量单位；FP16/TF32/BF16 在 Tensor Core 上可获得更高吞吐。
- **特殊函数**：`sin`、`cos`、`exp`、`log`、`pow`、`sqrt`、`rsqrt` 等通过 SFU（Special Function Unit）执行，吞吐低于普通 ALU。如果精度允许，使用 intrinsic 版本（如 `__sinf`、`__expf`）更快但精度较低。
- **FMA**：乘加融合指令 `fmaf` 通常比单独 `mul` + `add` 更快且精度更高。

```cuda
// 推荐：使用 FMA 风格，编译器更容易生成 fma 指令
float c = a * b + d;
```

###### 5.3.2 控制流与分支发散

Warp 内所有线程共享同一个 PC（Program Counter）。如果 Warp 内线程走不同分支，GPU 会**串行执行**各个分支路径，导致性能下降。
...
- 让条件基于 **block 或 warp 级别**，而非 thread 级别。
- 使用 `__ballot_sync`、`__any_sync` 等 Warp Vote 函数检测分支情况。
- 对循环进行展开或调整粒度，保证 Warp 内行为一致。

###### 5.3.3 同步指令

`__syncthreads()` 会阻塞 block 内所有线程直到全部到达。过度同步会导致 SM 空闲。减少 shared memory kernel 中不必要的同步点，或通过 warp-level primitives 替代 block-level 同步。

---

##### 5.4 现代 GPU 上的高级性能特性

近年来 CUDA Programming Guide 增加了很多面向新架构的优化建议，了解它们能让你在 Ampere、Hopper 及以后架构上写出更高效的代码。

###### 5.4.1 异步拷贝（Asynchronous Data Copy）

从 Compute Capability 8.0（Ampere）开始，支持 `cp.async` 指令，允许 GPU 在 load global → shared memory 时无需等待，直接返回并继续执行其他 warp，数据传输由硬件异步完成。
...
__pipeline_memcpy_async(smem + threadIdx.x, gmem + globalIdx, sizeof(float));
__pipeline_commit();
__pipeline_wait_prior(0);
__syncthreads();
```

这种方式把 global memory load 和 shared memory 之间的延迟隐藏起来，是 CUTLASS、FlashAttention 等高性能 kernel 的核心技术之一。

###### 5.4.2 Tensor Memory Accelerator（TMA）

Compute Capability 9.0（Hopper）引入了 TMA，可以在硬件层面完成复杂的多维张量搬运，把地址计算和数据搬移从 CUDA core 卸载到专用硬件单元。TMA 通常与 Asynchronous Transaction Barrier（`mbarrier`）一起使用，实现更细粒度的异步数据流水线。

###### 5.4.3 DPX Instructions

Hopper 引入的 DPX（Dynamic Programming X）指令集，用于加速动态规划类算法，例如序列比对、图遍历中的 `max-plus`、`min-plus` 操作。

```cuda
int result = __vmaxs4(a, b);  // 示例：向量化的 max 操作
```

###### 5.4.4 Warp-Level Primitives

从 Compute Capability 3.0 起，CUDA 提供 warp shuffle 函数，允许同一 warp 内线程直接交换寄存器中的数据，无需经过 shared memory：

```cuda
// 从 lane 0 广播到整个 warp
float val = __shfl_sync(0xFFFFFFFF, localVal, 0);
...
这对归约、扫描、矩阵乘等算法非常有用，既能减少 shared memory 用量，也能降低同步开销。
...
| 优化方向 | 具体做法 | 适用场景 |
|---------|---------|---------|
| 内存合并访问 | 让 Warp 内线程按连续地址访问 global memory | 几乎所有 global memory 读写 |
...
| 异步拷贝 | 使用 `cp.async` / pipeline | Ampere 及以上、数据流水线 |
...
| 选择合适精度 | FP16/BF16/TF32 + Tensor Core | 深度学习、矩阵计算 |
| 减少特殊函数 | 用 intrinsic 或查表替代 sin/exp 等 | 计算密集型 kernel |

---

##### 5.6 与 Day 4–Day 6 的关联

| 本章小节 | 对应实践日 | 对应内容 |
|---------|-----------|---------|
| 5.1 / 5.2.2 Memory Coalescing | Day 4 | 矩阵转置、向量加法中的 global memory 访问模式 |
| 5.2.3 Shared Memory / Bank Conflict | Day 5 | 共享内存 tiling、bank conflict 检测与消除 |
| 5.1.3 Occupancy | Day 2 / Day 6 | Occupancy Calculator、Nsight Compute 分析 |
| 5.3 Instruction Throughput | Day 6 | Nsight Compute 指标、指令瓶颈分析 |
| 5.4 异步拷贝 / TMA | 进阶 | CUTLASS、FlashAttention、Hopper 新特性 |

---

##### 5.7 阅读建议

1. **第一遍快速通读**：不要纠结每个细节，重点理解“利用率、内存吞吐、指令吞吐”三大优化方向。
2. **第二遍带着问题读**：学完 Day 4–Day 6 后，再回来精读 5.2（内存）和 5.3（指令）。
3. **结合官方代码示例**：CUDA Samples 中的 `transpose`、`reduction`、`matrixMul`、`convolutionSeparable` 都是本章理论的具体实现。
4. **使用工具验证**：每做一个优化，都用 Nsight Compute / Nsight Systems 看实际指标变化，避免“感觉优化了其实没效果”。
5. **记录自己的 Cheat Sheet**：把针对自己 GPU 架构的关键数字（峰值算力、显存带宽、L2 大小、shared memory/bank 数）整理出来，调优时随时查阅。

> **推荐延伸阅读**：
> - CUDA C Programming Guide 官方文档：https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#performance-guidelines
> - CUDA Best Practices Guide：https://docs.nvidia.com/cuda/cuda-c-best-practices-guide/
> - 对应 Nsight Compute 文档中的 *Memory Workload Analysis*、*Occupancy*、*Instruction Statistics* 章节。
