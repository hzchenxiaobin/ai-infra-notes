...
$$AI = \frac{\text{FLOPs（浮点运算次数）}}{\text{Bytes（内存读写字节数）}}$$

GPU 有一个**平衡点（Ridge Point）**，由峰值算力和峰值带宽决定：

$$\text{Ridge Point} = \frac{\text{Peak FLOP/s}}{\text{Peak Bandwidth}}$$

- 当 $AI < \text{Ridge Point}$ 时 → **Memory-Bound**（算力富余，数据喂不饱）
- 当 $AI > \text{Ridge Point}$ 时 → **Compute-Bound**（数据充足，算力是瓶颈）
...
$$\frac{AI_{\text{element-wise}}}{\text{Ridge Point}} = \frac{0.083}{12.6} \approx 0.66\%$$

**结论**：element-wise 操作的计算强度只有平衡点的 **0.66%**！这意味着计算单元 99.3% 的时间在等数据。这就是为什么我们说它是**纯 memory-bound**。

##### 对比 GEMM（矩阵乘法）的计算强度

以 $C = A \times B$（$M=N=K=4096$，float32）为例：
